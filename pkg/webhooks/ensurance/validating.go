...
func validateNodeQualityProbe(nodeProbe ensuranceapi.NodeQualityProbe, fldPath *field.Path) field.ErrorList {
	...
	if (nodeProbe.NodeLocalGet == nil) && (nodeProbe.HTTPGet == nil) {
		allErrors = append(allErrors, field.Invalid(fldPath.Child("httpGet"), "", "The httpGet and nodeLocalGet must set one"))
		return allErrors
	}

	if (nodeProbe.NodeLocalGet != nil) && (nodeProbe.HTTPGet != nil) {
		allErrors = append(allErrors, field.Invalid(fldPath.Child("httpGet"), "", "The httpGet and nodeLocalGet can not set at the same time"))
		return allErrors
	}

	if nodeProbe.HTTPGet != nil {
		allErrors = append(allErrors, validateHTTPGetAction(nodeProbe.HTTPGet, fldPath.Child("httpGet"))...)
	}
...
func (p *ActionValidationAdmission) ValidateCreate(ctx context.Context, req runtime.Object) error {
	...
	allErrs := genericvalidation.ValidateObjectMeta(&action.ObjectMeta, false, genericvalidation.NameIsDNS1035Label, field.NewPath("metadata"))
