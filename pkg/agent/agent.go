...
func NewAgent(ctx context.Context, opts *options.Options) (*Agent, error) {
	...
	stateStoreManager := statestore.NewStateStoreManager(craneInformerFactory.Ensurance().V1alpha1().NodeQOSEnsurancePolicies().Informer(), podInformerFactory.Core().V1().Pods(), opts.Ifaces)
